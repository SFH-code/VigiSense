...
sensor::sensor(MAX30102 *s) {
	...
	fout.open("reportIRinterval.csv", std::ios::out|std::ios::app);
	fcalc.open("reportCalcLatency.csv", std::ios::out|std::ios::app);
...
void sensor::HRcalc() {
	std::cout<< "starting calculation for HR and SPO2"<<std::endl;
	if (runningHR) return;
	runningHR = true;
...
void sensor::loopThread() {
	while (runningHR) {
		...
		auto durationcalc = std::chrono::duration_cast<std::chrono::nanoseconds>(endcalc-startcalc).count();
		fcalc<<durationcalc<<"\n";
...
// Get data from sensor->and store in global variables
	...
	if (peakDetect(filteredIRValue)) {
		...
		R = ((MaxR-MinR)/MinR)/((MaxIR-MinIR)/MinIR);
		//R = ((localMaximaRed-localMinimaRed)/localMinimaRed)/((localMaximaIR-localMinimaIR)/localMinimaIR);
		//std::cout<< "--------R process data--------" << "\n" << "localMaximaRed:" <<localMaximaRed << "\n localMinimaRed:" << localMinimaRed << "\n localMaximaIR:" << localMaximaIR << "\n localMinimaIR:" << localMinimaIR << "\n R:" << R << "\n type R:" << typeid(R).name() <<std::endl;
		latestSpO2 = 104-17*R; 
...
bool sensor::peakDetect(int32_t data) {
	...
	//std::cout<< "localMaximaIR value:" << localMaximaIR << std::endl;
	//std::cout<< "localMinimaIR value:" << localMinimaIR << std::endl;
	//std::cout<< "crest value:" << crest << std::endl;
	//std::cout<< "trough value:" << trough << std::endl;
	if (data > localMaximaIR) {
		//std::cout<< "peak detect"<< std::endl;
		localMaximaIR = data;
		//std::cout<< "data value:" << data << std::endl;
		if (10 < (data - localMinimaIR)) { //might want to calibrate this everytime sensor is used			
			crest = true;			//filter red value only at crest and trough for efficiency
			...
			//std::cout << "Reatime manufactured localMaximaRed:" << localMaximaRed << std::endl;
		}
	}

	if (data < localMinimaIR) {
		//std::cout<< "trough detect"<< std::endl;
		localMinimaIR = data;
		if (crest && -10 > ((data)-(localMaximaIR))) {//Might want to calibrate this everytime sensor is used
			...
			//std::cout << "Reatime manufactured localMinimaRed:" << localMinimaRed << std::endl;
			//std::cout << "\n" << std::endl;
		}
	}
