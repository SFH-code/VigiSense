...
MAX30102::MAX30102() {
	// Constructor
	fMAX.open("reportISRLatency.csv", std::ios::out|std::ios::app);
}
...
void MAX30102::setup(uint8_t powerLevel, uint8_t sampleAverage, uint8_t ledMode, int sampleRate, int pulseWidth, int adcRange) {
	std::cout<<"Entering setup for MAX30102"<<std::endl;
	...
	gpioSetISRFuncEx(DEFAULT_INT_GPIO,FALLING_EDGE,10,gpioISR,(void*)this);
